...
class SolicitanteModel {
    ...
    public function guardarRegistro($solicitante) {
        if ($solicitante instanceof Solicitante) {
            $sql = "INSERT INTO solicitante (nombreCompleto, fechaRegistro, correo, telefono, direccion, cuidadDepId, sectorEconomicoId) " .
                    "VALUES (?, ?, ?, ?, ?, ?, ?);";
            try {
                $now = date("Y-m-d");
                ...
                $results->execute(array($solicitante->getNombreCompleto(), $solicitante->getFechaRegistro(),
                    $solicitante->getCorreo(), $solicitante->getTelefono(), $solicitante->getDireccion(), $solicitante->m_CiudadDep->getCiudadDepID(),
                    $solicitante->m_SectorEconomico->getSectorEconomicoID()));
